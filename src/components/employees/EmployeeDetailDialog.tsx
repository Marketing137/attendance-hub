    ...
    <Dialog open={open} onOpenChange={(open) => {
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] p-0">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              {contractType && (
                <Badge 
                  variant="outline"
                  style={{ borderColor: contractType.color, color: contractType.color }}
                >
                  {contractType.name}
                </Badge>
              )}
        ...
        <div className="flex flex-wrap gap-2 pb-4">
          ...
          {sanctions.length === 0 && (
            <Button size="sm" variant="outline" className="text-amber-600 border-amber-500" onClick={handleApplySanction}>
              ...
              >
                ...
                {daysUntilEnd !== null && daysUntilEnd <= 30 && daysUntilEnd > 0 && (
                  <div className="p-4 rounded-lg bg-amber-500/10 border border-amber-500/30 flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-amber-600">Contrato próximo a vencer</p>
                ...
                {daysUntilEnd !== null && daysUntilEnd <= 0 && (
                  <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/30 flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-red-600">Contrato vencido</p>
                      <p className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/30">
                    ...
                    <div>
                      ...
                      <Badge 
                        variant="outline"
                        style={{ 
                          borderColor: contractType?.color,
                  ...
                  <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/30">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-green-600">
                        S/. {contract.salary.toLocaleString('es-PE')}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/30">
                ...
                {contract.probationEndDate && (
                  <div className="p-4 rounded-lg bg-muted/30">
                    <h4 className="text-sm font-medium mb-2">Período de Prueba</h4>
                    <p className="text-sm text-muted-foreground">
                      Finaliza: {new Date(contract.probationEndDate).toLocaleDateString('es-PE')}
                      {new Date(contract.probationEndDate) < new Date() && (
                ...
                <div className="p-4 rounded-lg bg-muted/30">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge 
                      variant={contract.documentsComplete ? 'default' : 'destructive'}
                      className={contract.documentsComplete ? 'bg-green-500' : ''}
                    >
                      {contract.documentsComplete ? 'Completa' : 'Incompleta'}
                    </Badge>
                  </div>
                  {!contract.documentsComplete && (
                    <p className="text-sm text-muted-foreground mt-2">
                      <AlertCircle className="w-4 h-4 inline mr-1 text-amber-500" />
                      Solicitar documentos faltantes al colaborador
                    </p>
                  )}
                </div>
              ...
              {sanctions.length > 0 ? (
                <div className="space-y-3">
                  {sanctions.map((sanction) => (
                    <div 
                      key={sanction.id}
                      className="p-4 rounded-lg border border-red-200 bg-red-50/50 dark:border-red-900 dark:bg-red-950/20"
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <Badge 
                            variant="destructive"
                            className={
                              sanction.type === 'verbal_warning' ? 'bg-amber-500' :
                              sanction.type === 'written_warning' ? 'bg-orange-500' :
                              sanction.type === 'suspension' ? 'bg-red-500' : 'bg-red-700'
                            }
                          >
